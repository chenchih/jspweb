...
String sqlPagination="SELECT SQL_CALC_FOUND_ROWS * FROM member limit "+iPageNo+","+iShowRows+"";
...
<%
  ...
  %>
    <tr>
      ...
      <td><%=rsPagination.getString("userid")%></td>
      <td><%=rsPagination.getString("pwd")%></td>
    </tr>
    <% 
 }
 %>
...
<%
        ...
        if((cPage*iTotalSearchRecords)-(iTotalSearchRecords)>0)
        {
		 ...
		 a href="ss.jsp?iPageNo=<%=prePageNo%>&cPageNo=<%=prePageNo%>"> << Previous</a>
         <%
        }
        ...
        for(i=((cPage*iTotalSearchRecords)-(iTotalSearchRecords-1));i<=(cPage*iTotalSearchRecords);i++)
        {
          ...
          {
          %>
           <a href="ss.jsp?iPageNo=<%=i%>" style="cursor:pointer;color: red"><b><%=i%></b></a>
          <%
          }
          else if(i<=iTotalPages)
          {
          %>
           <a href="ss.jsp?iPageNo=<%=i%>"><%=i%></a>
          <% 
          }
        }
        ...
        {
         %>
         <a href="ss.jsp?iPageNo=<%=i%>&cPageNo=<%=i%>"> >> Next</a> 
         <%
        }
        }
      %>
